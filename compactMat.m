...
classdef compactMat
    ...
    methods
        ...
        %0 -> initializes empty matrix given size as [n_rows, n_cols]
        %0 -> initializes matrix of given size with given list of
        %elements, takes two arguments: first [n_rows, n_cols] and second a list of elements [[e1, r1, c1]; [e2, r2, c2];...]
        %1 -> initializes matrix of given size with given 2D matrix (convert into list of elements and
        %insert)
            ...
            switch f
                case 0  %Empty initialization
                    M.Size = sym(size);
                case 1 %Initialization with elements
                    M.Size = sym(size);
                    M = compactInsert(0, M, e);
                case 2 %Initialization from bi-dimensional matrix
                    M.Size = sym(size);

                    M = compactInsert(1, M, e);
        ...
        %0 -> insert stack of elements [elem, row, col]. Inserts data [[e1, r1, c1]; [e2, r2, c2];...] 
        ...
        %1 -> insert entire 2D matrix (convert into stack of elements and
        %insert)
        ...
        %such value in the column "where" of the data. If mode == 0 the
        %search is executed on DataInd (indexing my element), if mode == 1
        %it is executed on Data (indexing by row/cols)
        ...
        %A call compactFind(1, M, 1, 1) will return all elements [valuei, rowi, coli]
        %with valuei == 1. 
        % A call compactFind(1, M, 1, 2) returns all elements at the 1st row of the matrix.
        %A call compactFind(1, M, [1, 1], [1, 2]) will return all elements [valuei, rowi, coli]
        %with valuei == 1 && rowi == 1. 
        % A call compactFind(0, M, 1, 2) returns the first element of the
        ...
        %choices are sum,, subtraction (sub), multiplication (mul), division (div)
            ...
            S = compactMat(0, compactGetInfo(M).Size);
        ...
        %Also, the matrices must be bi-dimensional.
            ...
            P = compactMat(0,[M.Size(1), Ms.Size(2)]);
            ...
            for r = 1:length(uniqueRows)
                ...
                for c = 1:length(uniqueCols)
                    ...
                    P = compactInsert(0, P, [sum(nonZero(:, 1)), uniqueRows(r), uniqueCols(c)]);
                end
            end
            end
        end
